<template>
  ...
  >
    <span class="flex absolute" :style="textStyle"
      >{{ element.weight }} KG</span
    >
  </div>
</template>
<script>
...
export default {
  ...
  computed: {
    elementStyle() {
      ...
      return {
        color: lightenColor,
        transform:
          !this.element.isDropped && !this.element.isRandom
            ? `scale(${this.element.size})`
            : `scale(${this.element.size}) translateY(-100%)`,
        fontSize: `${
          this.element.size <= 1
            ? this.element.size / 1.5
            : this.element.size / 4
        }vw`,
      };
    },
  ...
  mounted() {
    ...
    this.$watch("element.left", this.setPosition, { immediate: true });
  },
  methods: {
    setPosition(left) {
      const { width } = this.$el.getBoundingClientRect();
      const start = this.element.isRandom ? 60 : 0;

      this.$el.style.left =
        this.element.isRandom && left > 25
